...
2. **读记忆**：读 [`TODO.md`](./TODO.md)（机器可读，了解未完成事项）和 [`LOG.md`](./LOG.md)（人类可读，了解历史决策与踩过的坑）。**这是你的"上一班同事的交接班记录"，不读就开干 = 失忆症。**
3. **读流程**：根据任务类型，打开对应 SOP：
   - 要为一个新项目搭 harness → [`GUIDE/harness-construction.md`](./GUIDE/harness-construction.md)
   - 不确定该用 subagent / agent team / skill / 还是 slash command → [`GUIDE/orchestration.md`](./GUIDE/orchestration.md)
4. **找模板**：所有产物都有模板，**不要从零写**，去 [`templates/`](./templates/) 复制后再裁剪。
5. **干活 + 记账**：每完成一件事，**同时**更新 `TODO.md`（结构化）和 `LOG.md`（叙述性）。这是硬性纪律，不是可选项。
6. **沉淀**：发现重复在做的事 → 按 [`GUIDE/orchestration.md`](./GUIDE/orchestration.md) 判断沉淀为 slash command（简单）还是 skill（复杂）。
...
├─ AGENTS.md                       # 【总指挥】给"构建者 agent"的最高约束
...
| **渐进式披露** | `AGENTS.md` 是 ~100 行的"目录/路由"，细节下沉到 `GUIDE/`、`references/`，避免上下文拥挤。 |
...
> 参考来源：OpenAI《Harness Engineering》、Anthropic 长时运行 agent 双体架构、Stripe / LangChain / Mitchell Hashimoto 一线实践。详见 [`references/`](./references/)。
